...
char const*const TRACKBALL_BLINK_FILE
	= "/sys/class/leds/jogball-backlight/blink";
...
char const*const VTKEY_FILE
        = "/sys/class/leds/vtkey-backlight/brightness";
        ...
        struct light_state_t const* state)
{
    ...
    err = write_int(VTKEY_FILE, on?255:0);
    pthread_mutex_unlock(&g_lock);
    return err;
}

static int
set_light_vtkey(struct light_device_t* dev,
        struct light_state_t const* state)
{
    int err = 0;
    int on = is_lit(state);
...
// here we add the codes for trackball blink
static int
        ...
        struct hw_device_t** device)
{
    ...
    else if (0 == strcmp(LIGHT_ID_VTKEY, name)) {
        set_light = set_light_vtkey;
    }
    else {
        return -EINVAL;
    }
